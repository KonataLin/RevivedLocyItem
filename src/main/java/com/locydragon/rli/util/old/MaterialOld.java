...
public enum MaterialOld
{
  ...
  private static MaterialOld[] byId;
  private static final Map<String, MaterialOld> BY_NAME;
  private final int maxStack;
  private final short durability;

  static { byId = new MaterialOld[383];
    BY_NAME = Maps.newHashMap();

    for (MaterialOld material : values()) {
      if (byId.length > material.id) {
        byId[material.id] = material;
      } else {
        byId = (MaterialOld[])Java15Compat.Arrays_copyOfRange(byId, 0, material.id + 2);
        byId[material.id] = material;
      }
  ...
  private MaterialOld(int id)
  {
    this(id, 64);
  }

  private MaterialOld(int id, int stack) {
    this(id, stack, MaterialData.class);
  }

  private MaterialOld(int id, int stack, int durability) {
    this(id, stack, durability, MaterialData.class);
  }

  private MaterialOld(int id, Class<? extends MaterialData> data) {
    this(id, 64, data);
  }

  private MaterialOld(int id, int stack, Class<? extends MaterialData> data) {
    this(id, stack, 0, data);
  }

  private MaterialOld(int id, int stack, int durability, Class<? extends MaterialData> data) {
    this.id = id;
    this.durability = ((short)durability);
    this.maxStack = stack;
  }

  public static MaterialOld getMaterial(int id)
  {
