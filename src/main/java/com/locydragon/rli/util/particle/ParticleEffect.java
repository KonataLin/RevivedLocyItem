...
import com.locydragon.rli.nms.ReflectionHandler;
...
public enum ParticleEffect {
	EXPLOSION_NORMAL(
            ...
            "explode", 0, -1), EXPLODE("explode", 0, -1), EXPLOSION_LARGE(

            "largeexplode", 1, -1), LARGE_EXPLODE("largeexplode", 1, -1), EXPLOSION_HUGE(

            "hugeexplosion", 2, -1), HUGE_EXPLOSION("hugeexplosion", 2, -1), FIREWORKS_SPARK(

            "fireworksSpark", 3, -1), WATER_BUBBLE(

            "bubble", 4, -1, false, true), BUBBLE("bubble", 4, -1, false, true), WATER_SPLASH(

            "splash", 5, -1), SPLASH("splash", 5, -1), WATER_WAKE(

            "wake", 6, 7), WAKE("wake", 6, 7), SUSPENDED(

            "suspended", 7, -1, false, true), SUSPEND("suspended", 7, -1, false, true), SUSPENDED_DEPTH(

            "depthSuspend", 8, -1), DEPTH_SUSPEND("depthSuspend", 8, -1), CRIT(

            "crit", 9, -1), CRIT_MAGIC(

            "magicCrit", 10, -1), MAGIC_CRIT("magicCrit", 10, -1), SMOKE_NORMAL(

            "smoke", 11, -1), SMOKE("smoke", 11, -1), SMOKE_LARGE(

            "largesmoke", 12, -1), LARGE_SMOKE("largesmoke", 12, -1), SPELL(

            "spell", 13, -1), SPELL_INSTANT(

            "instantSpell", 14, -1), INSTANT_SPELL("instantSpell", 14, -1), SPELL_MOB(

            "mobSpell", 15, -1), MOB_SPELL("mobSpell", 15, -1), SPELL_MOB_AMBIENT(

            "mobSpellAmbient", 16, -1), MOB_SPELL_AMBIENT("mobSpellAmbient", 16, -1), SPELL_WITCH(

            "witchMagic", 17, -1), WITCH_MAGIC("witchMagic", 17, -1), DRIP_WATER(

            "dripWater", 18, -1), DRIP_LAVA(

            "dripLava", 19, -1), VILLAGER_ANGRY(

            "angryVillager", 20, -1), ANGRY_VILLAGER("angryVillager", 20, -1), VILLAGER_HAPPY(

            "happyVillager", 21, -1), HAPPY_VILLAGER("happyVillager", 21, -1), TOWN_AURA(
            ...
            "reddust", 30, -1), RED_DUST("reddust", 30, -1), SNOWBALL(

            "snowballpoof", 31, -1), SNOWBALL_POOF("snowballpoof", 31, -1), SNOW_SHOVEL(
            ...
            "mobappearance", 41, 8), END_ROD(

            "endRod", 42, 9), DRAGON_BREATH(

            "dragonBreath", 43, 9), DAMAGE_INDICATOR(

            "damageIndicator", 44, 9), SWEEP(

            "sweepAttack", 45, 9), FALLING_DUST(

            "fallingDust", 46, 10, true);
    ...
    private static boolean isWater(Location location) {
        ...
        return (material == Material.WATER) || (material == Material.valueOf("STATIONARY_WATER"));
    }
    ...
    public void display(float offsetX, float offsetY, float offsetZ, float speed, int amount, Location center, double range)
        ...
        new ParticlePacket(this, offsetX, offsetY, offsetZ, speed, amount, true, null).sendTo(center, 256D);
    }

    public void display(Location center, float offsetX, float offsetY, float offsetZ, float speed, int amount) {
        display(offsetX, offsetY, offsetZ, speed, amount, center, 257.0D);
    }
    ...
    public static final class ColorData
            extends ParticleEffect.ParticleData {
        public ColorData(int R, int G, int B, byte data) {
            super(data);
    ...
    public static final class BlockData
        ...
        public BlockData(Material material, byte data)
            ...
            setPacketData(new int[]{
                    material.getId() + data * 4096});
        }
    }
    ...
    public static final class ParticlePacket {
        ...
        public static void initialize()
            ...
            try {
                ...
                Class<?> packetClass = ReflectionHandler.PackageType.MINECRAFT_SERVER
                ...
                playerConnection = ReflectionHandler.getField("EntityPlayer", ReflectionHandler.PackageType.MINECRAFT_SERVER, false,
                        "playerConnection");
